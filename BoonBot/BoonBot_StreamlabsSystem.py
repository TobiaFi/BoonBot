...
def end_poll():
    ...
    Parent.SendStreamMessage("Poll has ended. Boon #" + str(vote_count.most_common()[0][0]) + ' was picked with ' + str(vote_count.most_common()[0][1]) + ' vote(s).')
    votes = dict()
    vote_count = dict()
    return
...
def Execute(data):
    if data.IsChatMessage() and data.Message.lower() == '!boon' and data.UserName.lower() == 'tobiaf' and not poll_is_active:
        ...
        global poll_is_active
        poll_is_active = True
        t = threading.Timer(5.0, end_poll)
        t.start()
